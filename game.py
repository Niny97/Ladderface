...
class Game:
    def __init__(self, screen, objects, obj_rects, buttons, button_rects):
        ...
        self.collected_rect = self.collected_text.get_rect(topright=(28.8 * 32, 5))
        ...
        self.restart_button = Button(self.screen, self.button_rects[8], 244, 459, "restart", self.buttons, "restart")
        self.to_menu_button = Button(self.screen, self.button_rects[9], 498, 459, "menu", self.buttons, "menu")
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if self.hearts == 0:
                ...
                score = int(len(self.levels[self.lvl].cset) * 100 - self.levels[self.lvl].time * 2)

                end_menu = EndMenu(self.screen, defeat_image, [self.restart_button, self.to_menu_button], self.lvl, score)
                self.next = end_menu.run()
                self.is_running = False

                break

            self.time_since_last = time.time() - self.last_frame
            ...
            if self.levels[self.lvl].player.win:
                self.win_sound.play()
                score = int(len(self.levels[self.lvl].cset) * 100 - self.levels[self.lvl].time * 2 + 50 * self.hearts)

                end_menu = EndMenu(self.screen, win_image, [self.restart_button, self.to_menu_button], self.lvl, score)
                self.next = end_menu.run()
                self.is_running = False
    ...
    def render(self):
        ...
        self.levels[self.lvl].render(self.screen, self.levels[self.lvl].map, LAYERS, (32, 32))
        ...
        self.draw_hearts()
        self.screen.blit(self.collected_text, self.collected_rect)
        self.draw_time()
        pygame.display.flip()
